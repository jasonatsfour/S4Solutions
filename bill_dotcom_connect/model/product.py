...
class ProductTemplate(models.Model):
    ...
    def sync_product(self):
        ...
        if not self.backend_record_id:
            ...
            dict_val = {"obj": {"entity": "Item","type" : "3", "name": str(self.name), "price" : self.list_price}}
            url = self.env['billdotcom.configure'].search([]).location
            login_end_point = str(url) + 'Crud/Create/Item.json'
            ...
            if result.json()['response_message'] == "Success":
                ...
                context['message'] = "Product synchronized Successfully!"
                res_dict = {
                    'name': "Success",
            ...
            headers = {'content-type': 'application/x-www-form-urlencoded'}
            dict_val = {"obj": {"entity": "Item", "id": str(self.backend_record_id),"type" : "3","name": str(self.name), "price" : self.list_price}}
            url = self.env['billdotcom.configure'].search([]).location
            login_end_point = str(url) + 'Crud/Update/Item.json'
            devkey = self.env['billdotcom.configure'].search([]).dev_key
            params_str = "devKey=" + str(devkey) + "&sessionId=" + str(session_id) + "&data=" + str(
                self.mydict(dict_val))
            result = requests.post(login_end_point, data=params_str, headers=headers)
            if result.json()['response_message'] == "Success":
                view = self.env.ref('sh_message.sh_message_wizard')
                view_id = view and view.id or False
                context = dict(self._context or {})
                context['message'] = "Synchronized Product Updated Successfully!"
                res_dict = {
                    'name': "Success",
                    'type': 'ir.actions.act_window',
                    'view_type': 'form',
...
class ProductProduct(models.Model):
    ...
    def sync_product(self):
        ...
        if not self.backend_record_id:
            ...
            headers = {'content-type': 'application/x-www-form-urlencoded'}
            dict_val = {"obj": {"entity": "Vendor", "id": str(self.backend_record_id),"type" : "3","name": str(self.name), "price" : self.list_price}}
            url = self.env['billdotcom.configure'].search([]).location
            login_end_point = str(url) + 'Crud/Update/Item.json'
            devkey = self.env['billdotcom.configure'].search([]).dev_key
            params_str = "devKey=" + str(devkey) + "&sessionId=" + str(session_id) + "&data=" + str(
